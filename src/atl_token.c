...
void token_merge(token_head_t *th, const char *p, size_t len, atl_token_type_t type ) {
    ...
    atl_token_t *t = (atl_token_t *)aml_pool_alloc(th->pool, sizeof(atl_token_t) + (tlen + len + 1));
    *t = *(th->tail);
    t->token = (char *)(t+1);
    t->type = type;
...
static atl_token_t * __token_init(token_head_t *th, const char *p, size_t len, atl_token_type_t type, atl_token_cb_t attr_type, atl_token_t *alt_token) {
    if(th->tail) {
    ...
    atl_token_t *resp = t;
    if(!t) {   
        t = (atl_token_t *)aml_pool_alloc(th->pool, sizeof(atl_token_t) + len+1);
        resp = t;
        memset(t, 0, sizeof(*t));
        t->token = (char *)(t+1);
        ...
        if(!th->tail->child && 
            ...
            if(tail_type == ATL_TOKEN_COLON) {
                ...
                th->tail->prev->prev->prev->prev && th->tail->prev->prev->prev->prev->prev && th->tail->prev->prev->prev->prev->prev->type == ATL_TOKEN_COMPARISON && th->tail->prev->prev->prev->prev->prev->prev) {
                    t = (atl_token_t *)aml_pool_alloc(th->pool, sizeof(atl_token_t) + 2);
...
static atl_token_t * _token_init(token_head_t *th, const char *p, size_t len, atl_token_type_t type, atl_token_cb_t attr_type) {
...
static atl_token_t * token_init(token_head_t *th, const char *p, size_t len, atl_token_type_t type) {
...
atl_token_t *atl_token_parse(aml_pool_t *pool, const char *s) {
    ...
    while(*s) {
        ...
        case ';':
            if(token_start) {
                token_init(&th, token_start, s-token_start, ATL_TOKEN_TOKEN );
                token_start = NULL;
            }
            s++;
            break;
        ...
        case 1 ... 32:
            ...
            while(*s > 0 && *s <= 32)
                s++;
            token_init(&th, " ", 1, ATL_TOKEN_SPACE);
            break;
        case '\\':
            if(token_start) {
                token_init(&th, token_start, s-token_start, ATL_TOKEN_TOKEN );
...
char *token_parse_attr(token_head_t *th, char *p, char *ep) {
    ...
    atl_token_t *tok = __token_init(th, data.param, strlen(data.param), ATL_TOKEN_TOKEN, v, data.alt_token);
    if(data.num_attrs) {
        ...
        */
        case '*':
            if(token_start) {
                if(has_parent(&th, ATL_TOKEN_OPEN_BRACE)) {
                    if(token_start) {
                        token_init(&th, token_start, s-token_start, ATL_TOKEN_MODIFIER );
                        token_start = NULL;
                    }                
                    token_init(&th, s, 1, ATL_TOKEN_OPERATOR);
                    s++;
                }
                else {
                    s++;
        ...
        case ':' :
            if(token_start) {                
                ...
                token_init(&th, s, 1, ATL_TOKEN_COLON);
                s++;
            }
            break;
        case '?' :
            ...
            token_init(&th, s, 1, ATL_TOKEN_QUESTION);
            s++;
            break;
        case '&' :
            if(token_start) {
            ...
            if(s[1] == '&')
                s++;
            token_init(&th, s, 1, ATL_TOKEN_AND );
            s++;
            break;
        case '|' :
            if(token_start) {
            ...
            if(s[1] == '|')
                s++;
            token_init(&th, s, 1, ATL_TOKEN_OR );
            s++;
            break;
        case '\"':
        {
            ...
            token_init(&th, s, 1, ATL_TOKEN_DQUOTE );
            s++;
            break;
        }
        case '.':
            ...
            if(s[1] == '.' && s[2] != 0) {
                token_init(&th, s, 2, ATL_TOKEN_DASH);
                s += 2;
            }
            else {
                token_init(&th, s, 1, ATL_TOKEN_OTHER);
                s++;
            }
            break;     
        case ',' :
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_COMMA);
                s++;
                break;
            }
        case '(':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_OPEN_PAREN);
                s++;
                break;
            }
        case ')':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_CLOSE_PAREN);
                s++;
                break;
            }
        case '{':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_OPEN_BRACE);
                s++;
                break;
            }
        case '}':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_CLOSE_BRACE);
                s++;
                break;
            }
        case '[':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_OPEN_BRACKET);
                s++;
                break;
            }
        case ']':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_CLOSE_BRACKET);
                s++;
                break;
            }
        case '!':
        ...
        case '>':
            if(!is_quoted(&th)) {
                ...
                token_init(&th, s, 1, ATL_TOKEN_COMPARISON);
                s++;
                break;
            }
        case '-':
